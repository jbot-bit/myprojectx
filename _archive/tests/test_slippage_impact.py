...
Test how system performs with fixed slippage:
- 1 tick slippage (realistic)
- 2 ticks slippage (conservative)

Slippage applied:
- Entry: Worse price by N ticks
- Stop: Worse price by N ticks (stop slippage)
- Target: No slippage (assume limit order)

Measures if edge survives transaction costs.
"""
...
def simulate_orb_with_slippage(con, orb_time, rr, sl_mode, slippage_ticks, filters=None):
    """
    Simulate ORB with fixed slippage on entry and stop.

    slippage_ticks: 0, 1, or 2
    """
    if filters is None:
        filters = {}
    ...
    for _, setup in df_setups.iterrows():
        ...
        # Apply entry slippage (worse price)
        entry_price = trade_bars.loc[entry_bar_idx, 'close']

        if direction == 'UP':
            entry_price += slippage_ticks * TICK_SIZE  # Buy higher
            if sl_mode == 'HALF':
                stop = orb_midpoint
            else:
                stop = orb_low
            # Apply stop slippage (stop gets hit earlier)
            stop += slippage_ticks * TICK_SIZE
            risk = entry_price - stop
            target = entry_price + rr * risk  # No slippage on target (limit order)
        else:
            entry_price -= slippage_ticks * TICK_SIZE  # Sell lower
            if sl_mode == 'HALF':
                stop = orb_midpoint
            else:
                stop = orb_high
            ...
            target = entry_price - rr * risk  # No slippage on target
...
def main():
    ...
    print("=" * 80)
    print()
    print("Testing fixed slippage:")
    print("  0 ticks: No slippage (baseline)")
    print("  1 tick:  Realistic (entry + stop)")
    print("  2 ticks: Conservative (entry + stop)")
    print()
    print("Assumptions:")
    print("  - Entry: Worse price by N ticks")
    print("  - Stop: Worse price by N ticks (slips against you)")
    print("  - Target: No slippage (limit order filled at price)")
    print()
    ...
    slippage_tests = [0, 1, 2]
    all_results = []

    for slippage in slippage_tests:
        print(f"Testing {slippage} tick slippage...")
        ...
        for _, opt_row in optimal_df.iterrows():
            ...
            trades = simulate_orb_with_slippage(con, orb, rr, sl_mode, slippage, filters)
    ...
    results_df.to_csv('slippage_impact_results.csv', index=False)
    ...
    for orb in optimal_df['orb'].unique():
        ...
        baseline = orb_results[orb_results['slippage_ticks'] == 0]
        slip1 = orb_results[orb_results['slippage_ticks'] == 1]
        slip2 = orb_results[orb_results['slippage_ticks'] == 2]

        if len(baseline) > 0:
            baseline_r = baseline.iloc[0]['avg_r']
            print(f"  Baseline (0 ticks): {baseline_r:+.3f}R avg")

            if len(slip1) > 0:
                slip1_r = slip1.iloc[0]['avg_r']
                degradation1 = slip1_r - baseline_r
                pct1 = (degradation1 / baseline_r * 100) if baseline_r != 0 else 0
                print(f"  1 tick slippage:    {slip1_r:+.3f}R avg ({degradation1:+.3f}R, {pct1:+.1f}%)")

            if len(slip2) > 0:
                slip2_r = slip2.iloc[0]['avg_r']
                degradation2 = slip2_r - baseline_r
                pct2 = (degradation2 / baseline_r * 100) if baseline_r != 0 else 0
                print(f"  2 tick slippage:    {slip2_r:+.3f}R avg ({degradation2:+.3f}R, {pct2:+.1f}%)")

        print()

    print("=" * 80)
    print("TRANSACTION COST ROBUSTNESS")
    print("=" * 80)
    print()

    baseline_avg = results_df[results_df['slippage_ticks'] == 0]['avg_r'].mean()
    slip1_avg = results_df[results_df['slippage_ticks'] == 1]['avg_r'].mean()
    slip2_avg = results_df[results_df['slippage_ticks'] == 2]['avg_r'].mean()

    print(f"System-wide average:")
    print(f"  No slippage:  {baseline_avg:+.3f}R")
    print(f"  1 tick slip:  {slip1_avg:+.3f}R ({(slip1_avg - baseline_avg):+.3f}R, {((slip1_avg - baseline_avg) / baseline_avg * 100):+.1f}%)")
    print(f"  2 tick slip:  {slip2_avg:+.3f}R ({(slip2_avg - baseline_avg):+.3f}R, {((slip2_avg - baseline_avg) / baseline_avg * 100):+.1f}%)")
    print()

    # Verdict
    print("VERDICT:")
    if slip1_avg > 0:
        degradation_pct = abs((slip1_avg - baseline_avg) / baseline_avg * 100)
        print(f"  1 tick slippage: SURVIVES ({degradation_pct:.1f}% degradation, still profitable)")
    else:
        print(f"  1 tick slippage: FAILS (edge disappears)")

    if slip2_avg > 0:
        degradation_pct = abs((slip2_avg - baseline_avg) / baseline_avg * 100)
        print(f"  2 tick slippage: SURVIVES ({degradation_pct:.1f}% degradation, still profitable)")
    else:
        print(f"  2 tick slippage: FAILS (edge disappears)")

    print()
    print("Recommendation:")
    if slip1_avg > 0:
        print("  System is COST-ROBUST (edge survives realistic slippage)")
        print("  Real-world fills should be viable")
    else:
        print("  System is COST-SENSITIVE (thin edge, vulnerable to costs)")
        print("  WARNING: Backtest edge may not survive real-world execution")

    print()
    print("Files saved: slippage_impact_results.csv")
    print()
