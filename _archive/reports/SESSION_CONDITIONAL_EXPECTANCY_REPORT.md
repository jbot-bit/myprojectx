# Session Conditional Expectancy Analysis Report
...
**Key Finding**: Several sweep-based conditions show strong directional bias, with continuation strategies outperforming baseline by +0.15R to +0.24R. Reversal trades after Asia sweeps high are particularly toxic (-0.37R).
...
- **Asia labels → NY ORBs (2300, 0030)**
- **London labels → NY ORBs (2300, 0030)**
...
### 4. London ORB Outcome=Reject → NY 2300 (ANY) ❌ NEGATIVE
**Condition**: `london_orb_outcome=reject`
**ORB**: NY 2300
...
### 5. Asia Sweep Low → NY 2300 DOWN (CONTINUATION) ✅ POSITIVE
...
### 6. Asia Sweep High → London UP (CONTINUATION) ✅ POSITIVE
...
### 7. Asia Sweep Low → London DOWN (CONTINUATION) ✅ POSITIVE
...
### 8. Asia Sweep Low → NY 0030 UP (REVERSAL) ❌ NEGATIVE
...
### 9. Asia Failure=False → London (ANY) ❌ NEGATIVE
...
### 10. Asia Failure=True → London (ANY) ✅ POSITIVE
...
### 11. Asia Sweep Low → NY 0030 DOWN (CONTINUATION) ⚠️ MARGINAL
...
### Compression & Release (London Reject Effect)

When London ORB **rejects** (no break):
- **NY 0030**: +0.28R (explosive)
- **NY 2300**: -0.23R (poor)

**Interpretation**: Compressed range during London releases at **early** NY session (0030), not late NY session (2300). This suggests:
- If London was range-bound, trade **NY 0030**, not NY 2300.
- Energy builds during London and releases quickly.
...
3. **Trade NY 0030 when London rejected** (compression release)
4. **Favor London ORB when Asia was choppy** (failure = coiling)
...
**Strengths:**
- Large sample sizes (3,800 to 46,000 trades per ORB)
...
- Sweep definitions use post-Asia window (1100-1800), not intra-Asia
...
**London rejection is a leading indicator.** Range compression during London predicts explosive NY 0030 ORB setups (+0.28R).

**Actionable for discretionary traders:** Use prior-session context to filter ORB setups. Avoid toxic patterns (Asia sweep high → London SHORT). Favor high-probability patterns (sweep continuations, compression releases).
...
**Generated by**: `analyze_session_conditional_expectancy.py`
