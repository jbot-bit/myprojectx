...
def run_backtest(
    ...
    if reset:
        con.execute("DROP TABLE IF EXISTS orb_trades_5m_exec_nofilters")
    ensure_schema(con)
...
def main():
    ...
    p.add_argument("--reset", action="store_true", help="DROP and recreate orb_trades_5m_exec_nofilters (schema change fix)")
    args = p.parse_args()
