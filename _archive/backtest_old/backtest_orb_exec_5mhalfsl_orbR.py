...
# CORRECTED: 1R = ORB range size (not entry-to-stop distance)
...
def ensure_schema(con: duckdb.DuckDBPyConnection):
    con.execute(
        """
        CREATE TABLE IF NOT EXISTS orb_trades_5m_exec_orbr (
...
def run_backtest(
    ...
    if reset:
        con.execute("DROP TABLE IF EXISTS orb_trades_5m_exec_orbr")
    ensure_schema(con)
    ...
    print(
        f"\nRUN 5m (ORB-R mode): confirm={close_confirmations} | SL={sl_mode} | buffer_ticks={buffer_ticks} "
        f"| max_stop={MAX_STOP_TICKS} | RR={rr} | asia_tp_cap={ASIA_TP_CAP_TICKS}"
    )
    print(f"1R = ORB range size (high - low)")
    ...
    for idx, (d,) in enumerate(days, start=1):
        ...
        for orb, (h, m) in ORB_TIMES.items():
            ...
            # *** CRITICAL CHANGE: 1R = ORB range, not entry-to-stop ***
            orb_range_r = orb_range  # 1R defined as ORB size
            target_price = entry_price + rr * orb_range_r if direction == "UP" else entry_price - rr * orb_range_r
            ...
            for _, hi, lo, _cl in bars[entry_idx + 1 :]:
                ...
                if direction == "UP":
                    ...
                    if hit_target:
                        outcome = "WIN"
                        # WIN: Target was set to RR × ORB_range, so r_mult = RR
                        r_mult = float(rr)
                        break
                    if hit_stop:
                        outcome = "LOSS"
                        # LOSS: Actual loss in ORB-R units (actual_loss / ORB_range)
                        actual_loss_ticks = abs(entry_price - stop_price) / TICK_SIZE
                        orb_range_ticks = orb_range / TICK_SIZE
                        r_mult = -actual_loss_ticks / orb_range_ticks
                        break
                else:
                    ...
                    if hit_stop:
                        outcome = "LOSS"
                        # LOSS: Actual loss in ORB-R units (actual_loss / ORB_range)
                        actual_loss_ticks = abs(stop_price - entry_price) / TICK_SIZE
                        orb_range_ticks = orb_range / TICK_SIZE
                        r_mult = -actual_loss_ticks / orb_range_ticks
                        break

            entry_delay_bars = entry_idx + 1
            # MAE/MFE still relative to actual stop
            mae_r = (max_adv_ticks / stop_ticks) if stop_ticks > 0 else None
            mfe_r = (max_fav_ticks / stop_ticks) if stop_ticks > 0 else None

            # 18 cols => 18 placeholders (added orb_range_ticks)
            con.execute(
                """
                INSERT OR REPLACE INTO orb_trades_5m_exec_orbr
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                [
                    d,
                    ...
                    float(target_price),
                    float(stop_ticks),
                    float(orb_range / TICK_SIZE),  # orb_range_ticks
                    outcome,
                    float(r_mult),
...
def main():
    ...
    p.add_argument("--reset", action="store_true", help="DROP and recreate orb_trades_5m_exec_orbr (schema change fix)")
    args = p.parse_args()
