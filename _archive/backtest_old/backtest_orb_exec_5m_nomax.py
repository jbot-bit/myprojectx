...
# RULES (locked)
MAX_STOP_TICKS = 999999  # NO MAX STOP
RR_DEFAULT = 2.0
ASIA_TP_CAP_TICKS = 999999  # NO ASIA TP CAP
...
def orb_scan_end_local(orb: str, d: date) -> str:
    ...
    For 5m bars, this is still fine.
    """
    ...
    if orb == "2300":
        return f"{d + timedelta(days=1)} 00:30:00"  # end NY Futures block
    if orb == "0030":
...
def ensure_schema(con: duckdb.DuckDBPyConnection):
    con.execute("""
        CREATE TABLE IF NOT EXISTS orb_trades_5m_exec_nomax (
            ...
            outcome VARCHAR,
            r_multiple DOUBLE,
            entry_delay_bars INTEGER,   -- 5m bars after ORB end until entry trigger
            mae_r DOUBLE,
            mfe_r DOUBLE,
...
def run_backtest(close_confirmations: int = 1, rr: float = RR_DEFAULT, commit_every_days: int = 10):
    ...
    print(f"\nRUN 5m: confirm={close_confirmations} | max_stop={MAX_STOP_TICKS} | RR={rr} | asia_tp_cap={ASIA_TP_CAP_TICKS}")
    ...
    for idx, (d,) in enumerate(days, start=1):
        ...
        for orb, (h, m) in ORB_TIMES.items():
            ...
            # ORB levels from daily_features_v2 (built from 1m ORB window)
            row = con.execute(f"""
            ...
            orb_high, orb_low = float(row[0]), float(row[1])
            ...
            # For 5m bars: we begin after (open+5). The next 5m bar close is typically at (open+10).
            start_min = m + 5
            ...
            # Entry logic: N consecutive 5m CLOSES outside ORB
            consec = 0
            direction = None
            entry_price = None
            entry_ts = None
            ...
            for _, high, low, _close in bars[entry_idx + 1:]:
                high = float(high)
                low = float(low)
            ...
            entry_delay_bars = entry_idx + 1  # 5m bars after ORB end until entry trigger
            ...
            con.execute("""
                INSERT OR REPLACE INTO orb_trades_5m_exec_nomax
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, [
                d, orb, close_confirmations, rr,
                '', 0,  # sl_mode, buffer_ticks (not used in this variant)
                direction, entry_ts,
