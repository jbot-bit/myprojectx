...
def detect_daily_close_column(con: duckdb.DuckDBPyConnection) -> Optional[str]:
    if not table_exists(con, "daily_features"):
        return None
    cols = list_columns(con, "daily_features")
...
def main():
    ...
    if not table_exists(con, "daily_features"):
        raise SystemExit("ERROR: Table 'daily_features' not found in DB.")

    df_cols = list_columns(con, "daily_features")
    required = [
        "date_local",
        f"orb_{orb}_outcome",
    ...
    if missing:
        raise SystemExit(f"ERROR: Missing columns in daily_features: {missing}")
    ...
    if daily_close_col:
        ...
        if not bars_table:
            print("Skipped: No daily close field found in daily_features AND no bars table with (ts + close) detected.")
            print("Fix options:")
            print("  1) Add day_close to daily_features, OR")
